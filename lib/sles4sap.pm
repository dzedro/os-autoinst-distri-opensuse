...
sub stop_hana {
    ...
    if ($method eq 'crash') {
        ...
        $self->run_cmd(cmd => $cmd, node => $args{node}, ignore_failure => $args{ignore_failure});
        # wait until pacemaker is ready
        assert_script_run("until ssh $args{node} crm status;do echo wait;sleep 2; done", 500);
    }
    else {
...
#    upload_logs('/hana/shared/HA1/HDB00/hana-node01/trace/DB_HA1');
